...
exports.forex_script = async () => {
    try {
        ...
        const forexBtn = await frame.$('button#\\31');
        forexBtn.click();

        while (true) {
            const watchlist = await frame.$$('div.tv-widget-watch-list');

            const coinlist = await watchlist[1].$$('div.tv-widget-watch-list__row');
            let coindata = {}
