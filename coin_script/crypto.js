...
exports.crypto_script = async () => {
    try {
        ...
        const cryptoBtn = await frame.$('button#\\33');
        cryptoBtn.click();

        while (true) {
            const watchlist = await frame.$$('div.tv-widget-watch-list');

            const coinlist = await watchlist[3].$$('div.tv-widget-watch-list__row');
            let coindata = {}
